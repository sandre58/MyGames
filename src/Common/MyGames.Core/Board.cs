// -----------------------------------------------------------------------
// <copyright file="Board.cs" company="Stéphane ANDRE">
...
public abstract class Board<TPiece> : IBoard
    where TPiece : IPiece
{
    ...
    protected Board(int rows, int columns, IDictionary<TPiece, BoardCoordinates> pieces)
        : this(rows, columns) => pieces.ForEach(x => InsertPiece(x.Key, x.Value));

    protected Board(Board<TPiece> board)
        : this(board.Rows.Count, board.Columns.Count) => board.Pieces.ForEach(x => InsertPiece(x, board.GetCoordinates(x)));
    ...
    public IEnumerable<TPiece> Pieces => _pieces.Keys;
    ...
    private void BuildBoard(int rows, int columns)
    {
        ...
        _columns.AddRange(_squares.GroupBy(x => x.Value.Column).ToDictionary(x => x.Key, x => new SquaresColumn<TPiece>(x.Key, [.. x.Select(y => y.Value).OrderBy(y => y.Row)])));
        _rows.AddRange(_squares.GroupBy(x => x.Value.Row).ToDictionary(x => x.Key, x => new SquaresRow<TPiece>(x.Key, [.. x.Select(y => y.Value).OrderBy(y => y.Column)])));
    }
    ...
    internal virtual void SetFrom(Board<TPiece> board)
    {
        ...
        board.Pieces.ForEach(x => InsertPiece(x, board.GetCoordinates(x)));
    }
    ...
    public override string ToString()
    {
        ...
        foreach (var row in Rows)
        {
            foreach (var square in row)
            {
                ...
                // Piece
                strBuilder.AppendFormat(CultureInfo.CurrentCulture, "{0,1}", !square.IsEmpty ? square.Piece?.ToString() : string.Empty);
        ...
        {
            strBuilder.Append(CultureInfo.CurrentCulture, $"{column} ");
        }
